...
psql -U "$user" -d "$db" -c "create user $creator_user password 'creator'" -c "grant create on schema public to creator"

# Starting migrations from his name
flag="0"
for version_directory in $( find /migrations -maxdepth 1 -type d | sort); do
    if [[ $version_directory == "./migrations/$version" ]]
    then
      flag="1"
    fi
    if [[ -d "$version_directory" && $version_directory != "./migrations" ]]
    then
